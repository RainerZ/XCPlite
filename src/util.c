...
#ifndef M_2PI
#define M_2PI (2.0 * M_PI) /* not defined by all platforms */
#endif
...
//
// Uses splitmix64 - passes all TestU01 BigCrush tests, same speed class
// as xorshift64.  State is thread-local so no locking needed in
// multi-threaded code.
//
// Range reduction via Lemire's method: (uint128 * max) >> 64
//   - avoids integer division, bias < max/2^64 (negligible).
/**************************************************************************/
