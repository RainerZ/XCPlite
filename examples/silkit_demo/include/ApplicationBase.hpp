...
class ApplicationBase {
  ...
  public:
    ...
    enum struct DefaultArg { Name, Uri, Log, Config, Async, Autonomous, Duration, AsFastAsPossible, Sleep };
  ...
  private:
    ...
    enum struct SystemControllerResult { Unknown, CoordinatedStart, SystemControllerAbort, UserAbort };
    ...
    enum struct UnleashWorkerThreadResult { Unknown, ParticipantStarting, UserAbort };
  ...
  private:
    void AddDefaultArgs(std::unordered_set<DefaultArg> excludedCommandLineArgs = {}) {
        _commandLineParser->Add<CommandlineParser::Flag>("help", "h", "-h, --help", std::vector<std::string>{"Get this help."});

        Arguments defaultArgs{};

        if (!excludedCommandLineArgs.count(DefaultArg::Name)) {
            _commandLineParser->Add<CommandlineParser::Option>(
                defaultArgName.at(DefaultArg::Name), "n", defaultArgs.participantName, "-n, --" + defaultArgName.at(DefaultArg::Name) + " <name>",
                std::vector<std::string>{"The participant name used to take part in the simulation.", "Defaults to '" + _arguments.participantName + "'."});
        }

        if (!excludedCommandLineArgs.count(DefaultArg::Uri)) {
            _commandLineParser->Add<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Uri), "u", defaultArgs.registryUri,
                                                               "-u, --" + defaultArgName.at(DefaultArg::Uri) + " <uri>",
                                                               std::vector<std::string>{"The registry URI to connect to.", "Defaults to '" + _arguments.registryUri + "'."});
        }

        if (!excludedCommandLineArgs.count(DefaultArg::Log)) {
            _commandLineParser->Add<CommandlineParser::Option>(
                defaultArgName.at(DefaultArg::Log), "l", "info", "-l, --" + defaultArgName.at(DefaultArg::Log) + " <level>",
                std::vector<std::string>{"Log to stdout with level:", "'trace', 'debug', 'warn', 'info', 'error', 'critical' or 'off'.", "Defaults to 'info'.",
                                         "Cannot be used together with '--" + defaultArgName.at(DefaultArg::Config) + "'."});
        }

        if (!excludedCommandLineArgs.count(DefaultArg::Config)) {
            _commandLineParser->Add<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Config), "c", "", "-c, --" + defaultArgName.at(DefaultArg::Config) + " <filePath>",
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::Async)) {
            _commandLineParser->Add<CommandlineParser::Flag>(
                ...
                std::vector<std::string>{"Run without time synchronization mode.", "Cannot be used together with '--" + defaultArgName.at(DefaultArg::Duration) + "'."});
        }

        if (!excludedCommandLineArgs.count(DefaultArg::Autonomous)) {
            _commandLineParser->Add<CommandlineParser::Flag>(defaultArgName.at(DefaultArg::Autonomous), "A", "-A, --" + defaultArgName.at(DefaultArg::Autonomous),
                                                             std::vector<std::string>{"Start the simulation autonomously.", "Without this flag, a coordinated start is performed",
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::Duration)) {
            ...
            _commandLineParser->Add<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Duration), "d", defaultValue,
                                                               "-d, --" + defaultArgName.at(DefaultArg::Duration) + " <us>",
                                                               std::vector<std::string>{"The duration of a simulation step in microseconds.", "Defaults to " + defaultValue + "us.",
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::AsFastAsPossible)) {
            _commandLineParser->Add<CommandlineParser::Flag>(defaultArgName.at(DefaultArg::AsFastAsPossible), "f", "-f, --" + defaultArgName.at(DefaultArg::AsFastAsPossible),
                                                             std::vector<std::string>{"Run the simulation as fast as possible.",
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::Sleep)) {
            ...
            _commandLineParser->Add<CommandlineParser::Option>(
                defaultArgName.at(DefaultArg::Sleep), "s", defaultValue, "-s, --" + defaultArgName.at(DefaultArg::Sleep) + " <ms>",
                std::vector<std::string>{"The sleep duration per work cycle in milliseconds.", "Default is no sleeping.",
    ...
    auto IsValidLogLevel(const std::string &levelStr) -> bool {
        ...
        return logLevel == "trace" || logLevel == "debug" || logLevel == "warn" || logLevel == "info" || logLevel == "error" || logLevel == "critical" || logLevel == "off";
    }
    ...
    void EvaluateDefaultArgs(std::unordered_set<DefaultArg> excludedCommandLineArgs) {
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::Name)) {
            if (_commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Name)).HasValue()) {
                _arguments.participantName = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Name)).Value();
            }
        }

        if (!excludedCommandLineArgs.count(DefaultArg::Uri)) {
            if (_commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Uri)).HasValue()) {
                _arguments.registryUri = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Uri)).Value();
            }
        }

        bool hasAsyncFlag = false;
        if (!excludedCommandLineArgs.count(DefaultArg::Async)) {
            _arguments.runAsync = hasAsyncFlag = _commandLineParser->Get<CommandlineParser::Flag>(defaultArgName.at(DefaultArg::Async)).Value();
        }

        bool hasAutonomousFlag = false;
        if (!excludedCommandLineArgs.count(DefaultArg::Autonomous)) {
            _arguments.runAutonomous = hasAutonomousFlag = _commandLineParser->Get<CommandlineParser::Flag>(defaultArgName.at(DefaultArg::Autonomous)).Value();
        }

        bool hasDurationOption = false;
        if (!excludedCommandLineArgs.count(DefaultArg::Duration)) {
            hasDurationOption = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Duration)).HasValue();
            if (hasDurationOption) {
                int durationUs = std::stoi(_commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Duration)).Value());
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::Sleep)) {
            hasSleepOption = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Sleep)).HasValue();
            if (hasSleepOption) {
                int sleepDurationMs = std::stoi(_commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Sleep)).Value());
        ...
        if (hasAsyncFlag && hasDurationOption) {
            std::cerr << "Error: Options '--" << defaultArgName.at(DefaultArg::Async) << "' and '--" << defaultArgName.at(DefaultArg::Duration) << "' cannot be used simultaneously"
                      << std::endl;
            _commandLineParser->PrintUsageInfo(std::cerr);
            exit(-1);
        }
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::AsFastAsPossible)) {
            _arguments.asFastAsPossible = hasAsFastAsPossibleFlag = _commandLineParser->Get<CommandlineParser::Flag>(defaultArgName.at(DefaultArg::AsFastAsPossible)).Value();
        }

        if (hasAsFastAsPossibleFlag && hasSleepOption) {
            std::cerr << "Error: Options '--" << defaultArgName.at(DefaultArg::AsFastAsPossible) << "' and '--" << defaultArgName.at(DefaultArg::Sleep)
                      << "' cannot be used simultaneously" << std::endl;
            _commandLineParser->PrintUsageInfo(std::cerr);
            exit(-1);
        }
        ...
        if (!excludedCommandLineArgs.count(DefaultArg::Log)) {
            hasLogOption = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Log)).HasValue();
        }

        bool hasCfgOption = false;
        if (!excludedCommandLineArgs.count(DefaultArg::Config)) {
            hasCfgOption = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Config)).HasValue();
        }

        if (hasLogOption && hasCfgOption) {
            std::cerr << "Error: Options '--" << defaultArgName.at(DefaultArg::Log) << "' and '--" << defaultArgName.at(DefaultArg::Config) << "' cannot be used simultaneously"
                      << std::endl;
            _commandLineParser->PrintUsageInfo(std::cerr);
            exit(-1);
        }
        ...
        if (hasAsFastAsPossibleFlag && hasCfgOption) {
            std::cerr << "Error: Options '--" << defaultArgName.at(DefaultArg::AsFastAsPossible) << "' and '--" << defaultArgName.at(DefaultArg::Config)
                      << "' cannot be used simultaneously" << std::endl;
            _commandLineParser->PrintUsageInfo(std::cerr);
            exit(-1);
        }
        ...
        if (hasCfgOption) {
            ...
            const auto configFileName = _commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Config)).Value();
            try {
                _participantConfiguration = SilKit::Config::ParticipantConfigurationFromFile(configFileName);
            } catch (const SilKit::ConfigurationError &error) {
                std::cerr << "Failed to load configuration '" << configFileName << "':" << std::endl << error.what() << std::endl;
                exit(-1);
            }
        } else {
            std::string configLogLevel{"Off"};
            if (!excludedCommandLineArgs.count(DefaultArg::Log)) {
                const auto logLevel{_commandLineParser->Get<CommandlineParser::Option>(defaultArgName.at(DefaultArg::Log)).Value()};
            ...
            if (!_arguments.runAsync && !_arguments.asFastAsPossible && !_sleepingEnabled) {
                ...
                double animationFactor = (_arguments.duration.count() > 0) ? 0.5 / (1e-9 * _arguments.duration.count()) : 1.0;
            ...
            try {
                ...
                std::cerr << "Failed to set configuration from string '" << configString << "', " << error.what() << std::endl;
                exit(-1);
            }
        }
    }
    ...
    void SetupParticipant() {
        _participant = SilKit::CreateParticipant(_participantConfiguration, _arguments.participantName, _arguments.registryUri);
    ...
    void SetupAsync() {
        ...
        _lifecycleService->SetStartingHandler([this]() { _startWorkPromise.set_value(UnleashWorkerThreadResult::ParticipantStarting); });
    ...
    void SetupSync() {
        ...
        _timeSyncService->SetSimulationStepHandler(
            [this](std::chrono::nanoseconds now, std::chrono::nanoseconds /*duration*/) {
                ...
                _participant->GetLogger()->Debug(ss.str());
  ...
  protected:
    ...
    ISystemMonitor *GetSystemMonitor() const { return _systemMonitor; }
  ...
  public:
    ...
    void SetupCommandLineArgs(int argc, char **argv, const std::string &appDescription, std::unordered_set<DefaultArg> excludedCommandLineArgs = {}) {
