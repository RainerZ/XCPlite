...
static_assert(configTICK_RATE_HZ >= 1000, "fastTask needs configTICK_RATE_HZ >= 1000 for a 1 ms FreeRTOS tick period");
...
static BaseType_t delayTaskUntil(TickType_t *lastWakeTime, uint32_t periodMs) {
#ifdef ARDUINO_ARCH_ESP32
    return xTaskDelayUntil(lastWakeTime, pdMS_TO_TICKS(periodMs));
#else
...
void fastTask(void *parameter) {
    ...
    for (;;) {
        ...
        const BaseType_t delayed = delayTaskUntil(&lastWakeTime, period_ms);
        if (delayed == pdFALSE) {
            fastTaskOverruns++;
        }
    }
...
void slowTask(void *parameter) {
    ...
    for (;;) {
        ...
        xcpDemoDisplayUpdate(slow_task_period_ms, counter, fast_task_period_ms, global_counter);

        digitalWrite(SLOWTASK_SCOPE_PIN, LOW);

        const BaseType_t delayed = delayTaskUntil(&lastWakeTime, slow_task_period_ms);
        if (delayed == pdFALSE) {
            slowTaskOverruns++;
        }
    }
